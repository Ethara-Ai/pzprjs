...
def _solve_tapa2(grid, rows, cols):
        ...
        for c in range(cols):
            ...
            for r in range(rows):
                if grid[r][c] is not None:
                    ...
                    unassigned += 1
            if unassigned == 0:
                if shaded_side <= unshaded_side:
                    return False
            else:
                if shaded_side + unassigned <= unshaded_side:
                    return False
        return True
    ...
    def _check_connectivity():
        ...
        start = next(iter(component_cells))
        visited = {start}
        queue = [start]
        while queue:
            cr, cc = queue.pop()
            for dr, dc in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
                nr, nc = cr + dr, cc + dc
                if (nr, nc) in component_cells and (nr, nc) not in visited:
                    visited.add((nr, nc))
                    queue.append((nr, nc))

        return len(visited) == len(component_cells)
    ...
    for r, c in empty_cells:
        adj_clues[(r, c)] = _adjacent_clues(r, c)

    def solve(idx):
        if idx == len(empty_cells):
            if not _check_column_majority():
                return False
        ...
        for val in (1, 0):
            ...
            if val == 1 and not _check_no_2x2(r, c):
                sol[r][c] = -1
                continue
...
def generate_custom_tapa2(difficulty="easy"):
    ...
    return {
        ...
        "metadata": {
            ...
            "modified_rules": [
                "no 2x2 fully shaded (tapa.js forbids 2x2 unshaded)",
                "shaded cells must be connected (tapa.js requires unshaded connected)",
                "each column: shaded count > unshaded count (tapa.js uses row majority)",
            ],
        },
        "created_at": now,
        "solution": moves,
...
if __name__ == "__main__":
    ...
    t0 = time.monotonic()
    puzzle_data = generate_custom_tapa2(difficulty)
